...
predictor_perf = data.frame(cbind(drugs_interest, Model=NA, ModelAUC=NA, DataCombination=NA, NbModelVars=NA, PercModelVars=NA, WtPercModelVars=NA, Score=NA, Probability=NA, Sensitivity=NA), stringsAsFactors=FALSE)
...
for (drug_compound in drugs_interest[[1]]) {
	#drug_compound=as.vector(drugs_interest[[1]][4]) #for testing purposes
...
#Apply cutoffs to limit results to only those drugs with sufficient input data, and good model AUCs
...
### Determine "recommended treatment", set to NA if none passed above threshold
...
cat ("Treatment & Model & AUC & Data & WPMV\\footnote[2] & Probability & Sensitivity \\\\ \\hline \n")
#Loop through and print row for each drug
for (i in 1:25){
drug_clean=gsub("_",x=predictor_perf[i,1],replacement="\\\\_")#Escapes "_" in drug names
cat (drug_clean," & ",predictor_perf[i,2]," & ",predictor_perf[i,3]," & ",predictor_perf[i,4]," & ",predictor_perf[i,7]," & ",predictor_perf[i,9]," & ",predictor_perf[i,10]," \\\\ \\hline \n")
}
cat ("\\end{tabular}\n")
cat ("\\footnotetext[2]{Weighted percent model variables present in input data}\n")
cat ("\\footnotetext[3]{Code location:",script_path,"}\n")
